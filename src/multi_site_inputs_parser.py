...
def add_load_profile_inputs(flat_dict, nested_dict, path_to_load_profile_folder="inputs/load_profiles"):
    """
    ...
    if "load_file" in flat_dict.keys():
        if not pd.isnull(flat_dict["load_file"]):  # case for some sites having custom load profiles
            fp = os.path.join(path_to_load_profile_folder, flat_dict['load_file'])
            load_profile_file = pd.read_csv(fp)
            
            load_profile_per_sqft = load_profile_file[flat_dict['load_profile_code']].tolist()
            
            if 'floor_sqft' in flat_dict.keys():
                floor_sqft = flat_dict['floor_sqft']
                load_profile = np.array(load_profile_per_sqft, dtype=float)* floor_sqft
...
def multi_site_csv_parser(path_to_csv, api_url, API_KEY, n_sites=None, start=None):
    """
    ...
    :param start: default=None. If integer value is passed then code will start processing from that site on.
    ...
    for i in range(len(df)):
        ...
        add_load_profile_inputs(site_inputs, posts[-1], path_to_load_profile_folder='inputs/load_profiles/')

        posts[i]['Scenario']['Site']['Wind'] = {'max_kw': 0}  # hack for Wind not in help endpoint
        posts[i]['Scenario']['Site']['CHP'] = {'max_kw': 0}
